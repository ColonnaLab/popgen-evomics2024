...
To calculate allele frequencies  African and American populations, we need to reference sample information from the metadata files. 
using the plink2 fileset, specifically focusing on SNPs and limiting the analysis to
These metadata files contain essential details about the samples' population affiliations, which allow us to selectively compute allele frequencies for the specified groups.

Calculate allele frequencies  only for the snps, only for africans and only for americans. To do this we need information about samples belonging to population., this info is contained in metadata files , use  the `plink2` fileset thatg we produced before and that is in `../data`
